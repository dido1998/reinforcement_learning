...
w11=tf.Variable(tf.truncated_normal([7,7,3,32],0.0,stddev=1e-1),name='w11')
h11=conv2d(w11,datav)
h11_r=tf.nn.relu(h11)

w12=tf.Variable(tf.truncated_normal([7,7,32,4],0.0,stddev=1e-1),trainable=True,name='w12')
...
iptornn=tf.reshape(h12_r,[-1,7*7*4])
...
w31v=tf.Variable(tf.truncated_normal([64,8],0.0,stddev=1e-1),name='w31v')
...
w31p=tf.Variable(tf.truncated_normal([64,8],0.0,stddev=1e-1),name='w31p')
...
prev_action=tf.placeholder(tf.float32,[None,1])
divisionofpolicy=tf.div(g_probs,prev_action)
clipping=tf.clip_by_value(divisionofpolicy,1-0.2,1+0.2)
finallclip=tf.minimum(tf.multiply(divisionofpolicy,advantage),tf.multiply(clipping,advantage))
lossp=-tf.reduce_sum(finallclip)+1e-4*entropy
loss=tf.add(lossp,0.01*lossv)

optimize=tf.train.AdamOptimizer(1e-6).minimize(loss)
...
saverprev=tf.train.Saver()
savernow=tf.train.Saver()
stp1=None
stp2=None
z1=False
g=False
        ...
        for seed1 in range(1): 
            ...
            for i in range(5000):
                ...
                while j<=200:
                    ...
                    if d==True:
                        if i==0:
                            savernow.save(sess, "/home/aniket/Desktop/manas-task-phase/gym-minigrid/ppomodels/modelnow.ckpt")
                            saverprev.save(sess,"/home/aniket/Desktop/manas-task-phase/gym-minigrid/ppomodels/modelprev.ckpt")
                        else:
                            saverprev.save(sess, "/home/aniket/Desktop/manas-task-phase/gym-minigrid/ppomodels/modelprev.ckpt")
                        ...
                        vs1=sess.run(v,feed_dict={data:d2})         
                        reward1=0
                        ...
                        savernow.restore(sess, "/home/aniket/Desktop/manas-task-phase/gym-minigrid/ppomodels/modelnow.ckpt")
                        op_new=sess.run(probs,feed_dict={data:d1})
                        previous_action=np.reshape(np.sum(op_new*d3,axis=1),[len(rs),1])
                        _,loss1=sess.run([optimize,loss],feed_dict={data:d1,target_actions:d3,advantage:adv,target_v:vs1,prev_action:previous_action})
                        print('loss:%f'%(loss1))
                        savernow.save(sess,"/home/aniket/Desktop/manas-task-phase/gym-minigrid/ppomodels/modelnow.ckpt") 
                        saverprev.restore(sess, "/home/aniket/Desktop/manas-task-phase/gym-minigrid/ppomodels/modelprev.ckpt")                        
                        if d==True:
                            rl.append(reward)
                            re=np.array(rl)
                            print(reward)
